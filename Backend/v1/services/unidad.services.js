...
export const agregarUnidad = async (
  equipoId,
  data = {}
) => {
  ...
  if (!equipo.codigo) {
    throw new Error(
      "El equipo no tiene código asignado"
    );
  }

  const existentes =
    await Unidad.countDocuments({
      equipo: equipoId,
    });
...
export const enviarAMantenimiento = async (
  ...
  const unidad =
    await Unidad.findById(unidadId);
  ...
  if (
    ...
    throw new Error(
      "Unidad ya está en mantenimiento"
    );
  }
  ...
  const prev =
    unidad.cantReparaciones || 0;

  unidad.cantReparaciones =
    prev + 1;
  ...
  unidad.historialMantenimiento.push(
    entry
  );
...
export const getGarantiaUnidad = async (
  id
) => {
  const unidad =
    await Unidad.findById(id);
...
export const getReparacionesUnidad =
  async (id) => {
    const unidad =
      await Unidad.findById(id);
...
export const asignarUnidad = async (
  ...
  const obra =
    await Obra.findById(ubicacionId);
  ...
  if (
    ...
    throw new Error(
      "No se puede asignar una unidad dada de baja"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "No se puede asignar una unidad en mantenimiento"
    );
  }
...
export const agregarUnidadesAEquipo = async ({
  equipoId,
  cantidad,
}) => {
  const equipo =
    await Equipo.findById(equipoId);
...
export const finalizarMantenimiento =
  async (id, usuario = null) => {
    const unidad =
      await Unidad.findById(id);
    ...
    if (
      ...
      for (
        ...
        const h =
          unidad.historialMantenimiento[i];
...
export const actualizarFechaCompra =
  async (
    id,
    fechaCompra
  ) => {
...
const obtenerIdsDisponiblesPorCantidad =
  async (
    equipoId,
    cantidad
  ) => {
...
export const ejecutarAccionMasivaPorCantidad =
  async ({
    equipoId,
    cantidad,
    accion,
    ...
    if (
      ...
      if (!obraId) {
        const error =
          new Error(
            "Debe indicar la obra de destino"
          );
    ...
    if (
      ...
      const error =
        new Error(
          "Debe proporcionar una fecha de compra válida"
        );
    ...
    const {
      ...
      await obtenerIdsDisponiblesPorCantidad(
        equipoId,
        cantidadNumero
      );
...
export const quitarUnidadDeObra =
  async (
    idUnidad,
    idObra
  ) => {
    const unidad =
      await Unidad.findById(
        idUnidad
      );
...
export const trasladarUnidadesAotraObra =
  async ({
    obraOrigenId,
    obraDestinoId,
    unidadesIds,
    ...
    const unidadesAMover =
      await Unidad.find(
        query
      );

    if (
      unidadesAMover.length ===
      0
    ) {
      throw new Error(
    ...
    if (
      obraDestino &&
      Array.isArray(
        obraDestino.unidades
      )
    ) {
      const nuevasFiltro =
        idsAMover.filter(
          (id) =>
            !obraDestino.unidades.some(
              (u) =>
                u.toString() ===
                id.toString()
            )
    ...
    return {
      ...
      registrosTrasladados:
        idsAMover.length,
    };
  };

export const actualizarDescripcionUnidad =
  async (
    id,
    descripcion
  ) => {
...
export const actualizarEtiquetaUnidad =
  async (
    id,
    etiqueta
  ) => {
